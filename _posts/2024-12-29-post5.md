...
Un filtro efectivo puede ser el siguiente, diseñado para capturar únicamente eventos de creación de procesos en directorios específicos, minimizando eventos irrelevantes:
...
Incluir: Captura la creación de procesos solo si provienen de directorios específicos (como C:\Program Files\ y C:\Windows\), que son más propensos a contener aplicaciones legítimas.
Excluir: Evita capturar eventos de procesos conocidos y legítimos que a menudo generan ruido, como PowerShell y CMD, a menos que se utilicen de forma sospechosa.
Conclusión
El uso de filtros avanzados en Sysmon es crucial para optimizar el rendimiento del sistema y mejorar la calidad de los logs. Un diseño cuidadoso de estos filtros permite a los analistas concentrarse en los eventos más relevantes, facilitando así la detección y respuesta ante incidentes.
