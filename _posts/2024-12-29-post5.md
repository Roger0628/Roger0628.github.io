...
El uso de filtros avanzados en Sysmon proporciona varias ventajas significativas en comparación con la captura indiscriminada de todos los eventos:

1. **Reducción del Ruido**: Los filtros permiten enfocarse en eventos relevantes, lo que ayuda a minimizar la cantidad de registros no útiles y a reducir el "ruido" en los logs.

2. **Mejor Rendimiento**: Al limitar la cantidad de eventos que Sysmon debe registrar, se reduce la carga en el sistema, lo que puede mejorar el rendimiento general y la eficiencia del procesamiento de logs.

3. **Facilitación del Análisis**: Con menos eventos a revisar, los analistas pueden concentrarse en los eventos críticos, lo que facilita la identificación de actividades sospechosas y mejora la respuesta ante incidentes.

4. **Optimización del Almacenamiento**: Filtrar eventos no relevantes puede ayudar a ahorrar espacio en disco, ya que se registran menos datos, lo que es crucial en entornos donde el almacenamiento es limitado.

5. **Mejor Calidad de los Logs**: Los filtros avanzados aseguran que solo se registren eventos significativos, mejorando así la calidad y la utilidad de los logs para el análisis forense.

## 4.1. Investiga cómo un mal diseño de filtros podría afectar el desempeño del sistema y la calidad de los logs.

Un mal diseño de filtros en Sysmon puede tener varios efectos negativos:

- **Sobrecarga del Sistema**: Si los filtros son demasiado amplios o no se configuran adecuadamente, se puede terminar capturando una gran cantidad de eventos irrelevantes, lo que puede generar una sobrecarga en el sistema y afectar el rendimiento general.

- **Dificultad para Analizar Logs**: Capturar demasiados eventos puede hacer que los logs se vuelvan abrumadores y difíciles de analizar. Esto puede llevar a que eventos críticos se pasen por alto o se ignoren debido a la saturación de información.
...
- **Desinformación**: La calidad de los logs se ve comprometida si se registran eventos irrelevantes. Esto puede llevar a decisiones erróneas basadas en información incorrecta o incompleta.
...
Un filtro sencillo pero efectivo podría ser el siguiente, diseñado para capturar solo eventos de creación de procesos que involucren ejecutables en rutas específicas, minimizando eventos irrelevantes:
